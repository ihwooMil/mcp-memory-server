...
_PATTERNS = LanguagePatterns(
    ...
    positive_feedback=[
        ...
        (r"(?i)\bthat'?s?\s+(?:what|exactly\s+what)\s+I\s+(?:said|meant)\b", 0.90, "that's what I said"),
    ...
    negative_feedback=[
        ...
        (r"(?i)\byou\s+(?:already\s+)?asked\s+(?:me\s+)?(?:that|this)\b", 0.85, "you already asked"),
    ...
    useful_feedback=[
        ...
        (r"(?i)\b(?:oh\s+)?(?:right|yeah),?\s+(?:I\s+)?(?:forgot|remember)\b", 0.75, "oh right I forgot"),
    ],
    ...
    # ── policy / StateEncoder ──
    question_pattern=r"[?\uff1f]|\b(?:what|where|when|who|how|why|which|do you|can you|could you|is it|are you|have you)\b",
    ...
    discourse_markers=[
        "by the way", "speaking of which", "anyway", "anyhow",
        "on another note", "that reminds me", "oh and",
        "also", "plus", "so", "well", "actually",
        "anyway though", "moving on", "changing the subject",
        "on a different note", "come to think of it",
        "that said", "but anyway",
    ],
    # ── resolution ──
    ...
    emphasis=[
        ...
        "totally", "completely", "quite", "especially", "truly", "definitely",
    ],
    preference_expressions=["always", "every time", "whenever", "without fail"],
    constraint_expressions=[
        "never", "absolutely not", "no way", "don't ever",
        "under no circumstances", "please don't",
    ],
    request_patterns=[
        "please", "could you", "can you", "would you", "I'd like",
        "I want", "I need",
    ],
    positive_feedback_simple=[
        "right", "correct", "exactly", "perfect", "yes", "that's right",
        "good", "great", "thanks", "thank you", "you remember",
    ],
    negative_feedback_simple=[
        "no", "wrong", "incorrect", "that's not right", "I never said that",
        "that's not what I said", "you're confused", "you misunderstood",
    ],
    clarification_patterns=[
        "what do you mean", "I don't understand", "could you explain",
        "what does that mean", "I'm confused", "can you clarify",
        "for example", "say that again",
    ],
    elaboration_patterns=[
        "tell me more", "go on", "continue", "and then",
        "what else", "more details", "can you elaborate",
    ],
    fact_utterance_patterns=["is", "am", "are", "was", "were"],
    preference_utterance_patterns=[
        "like", "love", "enjoy", "prefer", "hate", "dislike",
        "want", "wish", "hope",
    ],
    experience_utterance_patterns=[
        "did", "have done", "tried", "experienced", "visited",
        "used", "worked with", "been to",
    ],
    chars_per_token=4.0,
